...
Fluent est un [ORM](https://en.wikipedia.org/wiki/Object-relational_mapping) écrit et pensé pour Swift. Il tire parti de son système fortement typé pour offrir une interface intuitive vers votre base de données. La philosophie de Fluent est axée sur la création de types Swift appelés "modèles" qui représentent la structure des données de votre base. Ces modèles sont ensuite utilisés pour exécuter des opérations de création, lecture, mise à jour et suppression, plutôt que d'écrire des requêtes brutes.

## Configuration

### Nouveau projet

Lorsque vous créez un projet avec la commande `vapor new` (sans `-n`), répondez `yes` pour inclure Fluent et choisissez le pilote de base de données que vous souhaitez utiliser. Cela installera les dépendances nécessaires pour votre projet, et ajoutera un exemple de configuration dans le code.
...
- Un pilote Fluent (ou plus) correspondant à votre (vos) base(s) de données
...
Une fois ces packages ajoutés à votre liste de dépendances, vous pourrez configurer vos bases de données via `app.databases` dans `configure.swift`.
...
Chaque pilote Fluent (détaillés ci-dessous) aura sa propre configuration à ajouter.

### Pilotes

Fluent supporte actuellement quatre pilotes. Vous les retrouverez sur GitHub en cherchant le tag [`fluent-driver`](https://github.com/topics/fluent-driver), qui liste les pilotes officiels ainsi que des pilotes de tierce partie.

#### PostgreSQL

PostgreSQL est une base de données SQL open source. Elle est proposée chez la majorité des hébergeurs cloud. C'est le pilote **recommandé** pour Fluent.

Pour utiliser PostgreSQL, vous devrez ajouter les dépendances suivantes à votre package.
...
Vous devrez ensuite configurer Fluent pour qu'il puisse se connecter à votre de base de données en appelant `app.databases.use` dans `configure.swift`.
...
Vous pouvez également fournir ces informations au format chaîne de connexion de base de données comme ceci :
...
Pour utiliser SQLite, vous devrez ajouter les dépendances suivantes à votre package.
...
Vous pouvez aussi configurer SQLite pour stocker la base de données de façon éphémère en mémoire.
...
!!! danger "Perte de données"
    L'usage d'un stockage en mémoire offre de meilleures performances mais peut résulter en une perte de données. Assurez-vous de n'utiliser cette configuration uniquement pour des cas où vous n'avez pas besoin de conserver les données, comme les tests automatisés.

Si vous utilisez une base de données en mémoire, assurez-vous que la configuration de Fluent lance automatiquement les migrations, soit avec `--auto-migrate`, soit en ajoutant l'instruction `app.autoMigrate()` après vos migrations dans `configure.swift`.
...
MySQL est une base de données SQL open source populaire. Elle est proposée chez la majorité des hébergeurs cloud. Ce pilote pour MySQL supporte aussi MariaDB.

Pour utiliser MySQL, vous devrez ajouter les dépendances suivantes à votre package.
...
Pour configurer une connexion locale sans utiliser de certificat SSL, vous devriez désactiver la vérification de certificat. Vous pourriez  en avoir besoin si vous connectez une instance MySQL 8 sous Docker par exemple.
...
MongoDB est une base de données NoSQL populaire conçue pour les développeurs. Le pilote est compatible avec tous les hébergeurs cloud et installations auto-hébergées à partir de la version 3.4.

!!! note "Note"
    Ce pilote utilise un client MongoDB créé et maintenu par la communauté, [MongoKitten](https://github.com/OpenKitten/MongoKitten). MongoDB offre également un client officiel, [mongo-swift-driver](https://github.com/mongodb/mongo-swift-driver), ainsi qu'une intégration Vapor, [mongodb-vapor](https://github.com/mongodb/mongodb-vapor).
...
Le format chaîne de connexion standard de MongoDB à utiliser est [documenté ici](https://www.mongodb.com/docs/manual/reference/connection-string/).
...
Les modèles représentent les structures de données fixes de votre base, telles que des tables ou collections. Chaque modèle possède un ou plusieurs champs, qui stockent des valeurs codables. Tous les modèles ont également un identifiant unique. Des Property Wrappers sont utilisés pour déclarer les identifiants et champs, ainsi que des correspondances plus complexes que nous verrons par la suite. Observez le modèle suivant qui représente une galaxie.
...
Pour créer un nouveau modèle, il faut créer une classe qui se conforme au protocole `Model`.
...
La première exigence du protocole `Model` est la présence d'une chaîne statique nommée `schema`.
...
L'exigence suivante est la présence d'un champ d'identifiant nommé `id`.
...
Ce champ doit utiliser le Property Wrapper `@ID`. Fluent recommande d'utiliser le type `UUID` et la clé de champ spéciale `.id` qui sont compatibles avec tous les pilotes Fluent.
...
Après avoir ajouté l'identifiant, vous pouvez définir autant de champs que souhaité pour stocker des informations additionnelles. Dans cet exemple, le seul champ additionnel est le nom de la galaxie.
...
Pour des champs simples, le Property Wrapper `@Field` est utilisé. Tout comme `@ID`, le paramètre `key` indique le nom du champ en base de données. Son utilité se révèlera pour les cas où la convention de nommage diffère entre Swift et la base de données, comme l'usage de `snake_case` ou `camelCase`.
...
Enfin, vous pouvez définir un initialiseur pour définir les valeurs des propriétés de votre modèle.
...
Cela sert surtout pour l'évolution du modèle, si vous y ajoutez de nouvelles propriétés plus tard, car vous obtiendrez des erreurs à la compilation si la méthode init est modifiée sans que les appels soient mis à jour.

## Migrations

Si votre base de données utilise des schémas pré-définis, comme c'est le cas pour les bases de données SQL, vous aurez besoin d'une migration pour préparer la base pour votre modèle. Les migrations sont aussi utiles pour injecter des données en base. Pour créer une migration, définissez un nouveau type qui se conforme au protocole `Migration` ou `AsyncMigration`. Observez la migration suivante correspondant au modèle `Galaxy` précédent :

```swift
struct CreateGalaxy: AsyncMigration {
    // Prépare la base de données pour le stockage du modèle Galaxy.
...
La méthode `prepare` permet de préparer la base de données à stocker le modèle `Galaxy`.
...
Chaque champ ajouté au constructeur de schéma possède un nom, un type, et des contraintes facultatives.
...
Pour retourner directement un objet Galaxy depuis un contrôleur, vous devez le conformer au protocole `Content`.
...
`Galaxy.query` sert à créer un objet `QueryBuilder` (contructeur de requête) pour le modèle. `req.db` est une référence vers la base de données par défaut de votre application. Enfin, `all()` retourne tous les modèles stockés dans votre base.

Si vous compilez et lancez votre projet, puis faites une requête HTTP `GET /galaxies`, vous devriez obtenir un tableau vide. Ajoutons donc une route pour créer une galaxie.
...
Une fois que vous avez une instance de votre modèle, appeler `create(on:)` l'enregistrera dans la base. Cela retourne un `EventLoopFuture<Void>` qui signale la fin de l'enregistrement. Une fois enregistré, retournez-le avec `map`.
...
Une propriété `@Parent` est un champ qui stocke l'identifiant d'un autre modèle. Le modèle qui définit la référence est appelé "enfant", et le modèle référencé est appelé "parent". Ce type de relation s'appelle notoirement "one-to-many". Le paramètre `key` définit le nom de la colonne à utiliser pour stocker la clé du parent dans la table de l'enfant.
...
En mettant le préfixe `$` sur la propriété référençant le parent, vous avez accès à l'objet `PropertyWrapper` associé. Cette syntaxe est nécessaire pour accéder à l'objet `@Field` interne qui stocke la valeur réelle de l'identifiant.
...
struct CreateStar: AsyncMigration {
    // Prépare la base de données à stocker le modèle Star.
    func prepare(on database: Database) async throws {
...
Cette migration est quasiment identique à celles des galaxies, à l'exception du champ additionnel qui stocke l'identifiant de la galaxie parente.
...
Les pilotes Fluent émettent des logs des requêtes générées avec le niveau debug. Certains pilotes, comme FluentPostgreSQL, vous permettent de configurer ce niveau de log au moment de la configuration de la connexion à votre base de données.
...
Cela indique que l'application émettra tout log défini avec le niveau debug ou supérieur. Lorsque vous compilerez et exécuterez votre application, les déclarations générées par Fluent seront affichées sur la console.
